...
import { EVENT_TYPES } from '../../../lib/charge-profile-constants';

/**
 * EventRowsTable — "By Event" mode rows for the charge profile editor.
 *
 ...
 */
export default function EventRowsTable({ rows, isPercentage, onUpdateRow, onRemoveRow }) {
  return (
          ...
          {rows.length === 0 ? (
            ...
            <tr key={rIdx} className="border-t border-gray-100 dark:border-slate-800 align-top">
              <td className="px-2 py-2">
                <select value={row.event_type || ''} onChange={(e) => onUpdateRow(rIdx, 'event_type', e.target.value)}
              ...
              <td className="px-2 py-2">
                <OrgPicker value={row.event_location_id} valueLabel={row.event_location_label}
                  onChange={(org) => { onUpdateRow(rIdx, 'event_location_id', org?.id || null); onUpdateRow(rIdx, 'event_location_label', org?.name || ''); }}

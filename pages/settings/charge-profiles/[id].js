...
import {
  Plus, Trash2, X, Tag, Edit2, Copy, ChevronDown,
} from 'lucide-react';
...
// ── Lane Location Cell (Org / City,State / Zip) ─────────────
function LaneLocationCell({ typeValue, orgId, orgLabel, textValue, onTypeChange, onOrgChange, onTextChange, orgType, placeholder }) {
  return (
          ...
          <div className="border-t border-gray-200 dark:border-slate-700 pt-5">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {MATCH_RESOLUTION_OPTIONS.map((opt) => (
                <button key={opt.value} type="button" onClick={() => update('match_resolution', opt.value)}
                  className={`rounded-xl border p-3 text-left transition-all ${
                    form.match_resolution === opt.value

...
import { computeDurationSeconds } from './pricing-duration.js';
...
export function selectTier(tiers, mode, context) {
  ...
  switch (mode) {
    ...
    case 'by_event': {
      ...
      //
      // CRITICAL vocabulary note: the UI pickers (DRIVER_EVENT_TYPES /
      // EVENT_TYPES in the charge-profile-constants files) persist tier
      // event_type in UI vocabulary — e.g. 'PICK_UP_CONTAINER',
      // 'DELIVER_CONTAINER'. The routing events table stores event_type
      // in palette vocabulary — 'pull', 'pickup', 'deliver', etc. We
      // have to translate the tier side through legFromToEvent before
      // comparing, or the two vocabularies never meet and no tier ever
      // matches its own event (caught by Cowork during Plan C
      // verification — Plan B results that "passed" were false
      // positives flowing through the active[0] fallback).
      const events = context?.routingEvents || [];
      const hit = active.find((t) => {
        if (!t.event_type) return false;
        const want = legFromToEvent(t.event_type.toLowerCase());
      ...
      if (hit) return hit;
      // Fall back to active[0] ONLY when no tier has a REAL location filter.
      // When a tier explicitly scoped itself to a specific org / city / zip
      // and the load didn't match, falling back to the stored amount would
      // silently bill "Dallas-only" rates on Houston loads. Return null so
      // the engine reports $0 — the user's configured intent.
      //
      // CRITICAL: a tier with event_location_type set (e.g. 'org') but no
      // event_location_id AND no event_location_value is NOT a real filter —
      // the editor stores the type as a default UI selection ("Organization")
      // even before the user picks a specific value. Treating that as a real
      // filter caused profiles with no actual location scoping to silently
      // bill $0. Require an actual id or value to count.
      const anyHasLocationFilter = active.some((t) =>
        t.event_location_type && (t.event_location_id || t.event_location_value)
      );
    ...
    case 'by_leg': {
      ...
      //
      // Vocabulary translation: same story as by_event (fixed in 29218ea).
      // The UI's LEG_OPTIONS picker persists leg_from/leg_to in UI form
      // (sometimes UPPER_SNAKE from the DRIVER_EVENT_TYPES picker,
      // sometimes lower_snake from LEG_OPTIONS itself depending on
      // which UI path created the profile). Either way, legFromToEvent's
      // map keys are lowercase — so we lowercase FIRST, THEN look up.
      // Doing .toLowerCase() AFTER the lookup misses the map entirely
      // on uppercase inputs (map['PICK_UP_CONTAINER'] is undefined, falls
      // through to returning the input, then lowercasing to
      // 'pick_up_container' — never becomes 'pull'). Caught on Plan C
      // re-verification: B6 uppercase leg_from failed while Test D
      // lowercase leg_from worked.
      const events = context?.routingEvents || [];
      const hit = active.find((t) => {
        if (!t.leg_from || !t.leg_to) return false;
        const wantFrom = legFromToEvent(String(t.leg_from).toLowerCase());
        const wantTo   = legFromToEvent(String(t.leg_to).toLowerCase());
...
function resolveRadiusTier(tier, load) {
  ...
  const idx = radiusTiers.findIndex((entry) => {
    ...
    // Check end_distance for null BEFORE Number() coerces null to 0.
    // Null end_distance means "open-ended" — the final bracket covers
    // any distance above its start. If we Number()'d first, a null
    // end would become 0 and the bracket would only match miles === start.
    // (Caught by Cowork: 200 mi in a [150, null] bracket returned $0.)
    const rawEnd = entry?.end_distance;
    const hasUpperBound = rawEnd != null && !Number.isNaN(Number(rawEnd));
    const end = hasUpperBound ? Number(rawEnd) : null;
    const inRange = miles >= start && (end == null || miles <= end);
    return inRange;
  });
  ...
  if (rateType === 'per_unit') {
    return { amount_cents: Math.round(cents * miles), bracket_index: idx, isResolved };
  }

  // 'fixed' and 'percentage' both return cents as-is here.
  // Percentage resolution happens in the outer engine's second pass.
  return { amount_cents: cents, bracket_index: idx, isResolved };
}
...
export function resolveAmountCents({ tiers, calculation_mode, unit_of_measure }, context) {
  // Percentage tiers are SCALARS — the rate (e.g. 15.00%) doesn't depend on
  // whether routing events have happened yet. Bypass the per-mode selectTier
  // logic and just return the first active tier's amount_cents as-is. The
  // engine's percentage-pass will then compute the actual amount against the
  // matched base charge. Without this short-circuit, a by_event percentage
  // profile silently resolves to $0 when the gating event hasn't occurred,
  // because selectTier returns null and amount_cents falls through to 0.
  if (unit_of_measure === 'percentage') {
    const activeTiers = filterActiveByDate(tiers || []);
    const tier = activeTiers[0] || null;
    return {
      amount_cents: tier?.amount_cents || 0,
      minimum_amount_cents: tier?.minimum_amount_cents || 0,

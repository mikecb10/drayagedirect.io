...
function resolvePercentageCharges(charges) {
  for (const charge of charges) {
    ...
    if (baseCharge) {
      const baseCents = Math.max(baseCharge.amount_cents || 0, baseCharge.minimum_amount_cents || 0);
...
/**
 * Get the current active amount from a charge profile's tiers.
 */
function getProfileAmount(profile, load) {
  const tiers = profile.tiers || [];
  if (tiers.length === 0) return 0;

  const today = new Date().toISOString().slice(0, 10);

  // Find active tier by date range
  const activeTier = tiers.find((t) =>
    (!t.start_date || t.start_date <= today) &&
    (!t.end_date || t.end_date >= today)
  ) || tiers[0];

  return activeTier.amount_cents || 0;
}

/**
 * Get minimum amount from a charge profile's first tier.
 */
function getProfileMinimum(profile) {
  const tiers = profile.tiers || [];
  if (tiers.length === 0) return 0;
  return tiers[0].minimum_amount_cents || 0;
}
...
export async function applyChargesToLoad(svc, loadId, tenantId, charges) {
  ...
  // Check if load already has a charge set
  let { data: existingSets } = await svc
  ...
  // "Replace auto, keep manual" — remove existing auto-generated lines
  ...
  if (existingSets?.length > 0) {
    ...
    // Create a default charge set for the load using the shared naming helper
    // (keeps auto-created charge sets aligned with manual ones: CS_ORD_M000005)

...
export async function findMatchingCharges(svc, load, tenantId, opts = {}) {
  ...
  //
  // The stop_off_type join (migration 100) makes the behavior flags
  // (has_cargo_transfer, is_paid_to_driver, etc.) directly readable by
  // the new event_* rule primitives in condition-evaluator.js without
  // a second DB round-trip. Events with NULL stop_off_type_id simply
  // have stop_off_type = null on the joined row.
  ...
  // Rule conditions can key on these without each rule having to read
  // raw columns. All callers fetch the load with `*` or the update
  // `.select()` response, so both columns are already present on `load`.
...
function resolvePercentageCharges(charges) {
  for (const charge of charges) {
    ...
    // Sibling scan — if ANY sibling has unresolved distance, cascade before
    // checking baseCharge. Without this, when baseCharge is null (fallback
    // find excludes null-amount rows), charge.amount_cents retains its raw
    // percentage rate as dollars and persists silently.
    const hasUnresolvedSiblingAR = charges.some(
      (c) => c !== charge && c.needs_distance === true && c.amount_cents === null
    );
    if (hasUnresolvedSiblingAR) {

 ...
 *   - order      -> buildOrderTriggerContext (the legacy body, renamed)
 ...
 * routes legacy callers to the orders path with entityType='order' and
 * entityId=loadId.
 *
 * Returns a consistent shape for non-order builders:
 *   { variables: Record<string, string>, orderId: string | null }
 *
 * For orders (both object-arg and positional), ALSO returns the legacy
 * shape fields so existing callers (buildInvoiceContext, buildChargeSetContext,
 * fireTrigger) that destructure `{ context, formatPrefs, tenant, user, rawLoad }`
 * keep working unchanged. `variables` on the order path is an alias of
 * `context` (the full template-variable tree).
 *
...
export async function buildTriggerContext(svc, arg2, arg3, arg4) {
  ...
  // `null` has typeof 'object', so the leading truthiness check is load-bearing.
  const isObjectArg = arg2 !== null && typeof arg2 === 'object' && !Array.isArray(arg2);

  let tenantId, entityType, entityId, userId;
  if (isObjectArg) {
    ({ tenantId, entityType, entityId, userId } = arg2);
  } else {
    // Legacy positional: (svc, tenantId, loadId, userId)
    tenantId = arg2;
    entityType = 'order';
    entityId = arg3;
    userId = arg4;
  ...
  if (!tenantId || !entityId) {
    throw new Error(
      `buildTriggerContext: tenantId and entityId required (got tenantId=${tenantId}, entityId=${entityId})`
    );
  }

  const effectiveEntityType = entityType || 'order';
  switch (effectiveEntityType) {
    case 'order': {
      // Legacy order path — preserves the full { context, formatPrefs, tenant,
      // user, rawLoad } shape for existing callers, AND adds the
      // entity-aware `{ variables, orderId }` alias.
      const result = await buildOrderTriggerContext(svc, tenantId, entityId, userId);
      return {
        ...result,
        variables: result?.context || {},
        orderId: entityId,
      };
    }
    case 'charge_set':
    ...
    default:
      throw new Error(`buildTriggerContext: unknown entityType ${effectiveEntityType}`);
  }
}

/**
 * Build the full trigger context for a load (orders-only).
 *
 * Returns { context, formatPrefs, tenant, user, rawLoad } where:
 ...
 *   - rawLoad    = the full orders DB row with ALL columns (for umbrella matching)
 *
 * This is the legacy body of `buildTriggerContext` before Stream B.1c
 * generalization. It remains the implementation for the `entityType='order'`
 * branch of the router. New code should call `buildTriggerContext({ entityType,
 * entityId, ... })` instead.
 *
 * @internal
 */
...
/**
 * Build a template-rendering context for a bulk rate-con email covering N charge-sets.
 *
 * Mirror of buildBulkInvoiceContext: loads all charge-sets (validating
 * tenant ownership + expected count), reuses buildChargeSetContext for the
 * first charge-set to hydrate tenant/customer/load/format fields, then
 * decorates context.charge_set with bulk tokens:
 *   - charge_set.numbers      (comma-joined string, e.g. "CS-1001, CS-1002, CS-1003")
 *   - charge_set.count        (string)
 *   - charge_set.total_bulk   (dollars, summed and divided — catalog currency kind expects dollars)
 *
 * context.charge_sets = full array of charge-set rows (available for future tokens / debugging).
 *
 * NOTE: argument order mirrors buildBulkInvoiceContext: (svc, tenantId, chargeSetIds).
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} svc
 * @param {string} tenantId
 * @param {string[]} chargeSetIds - non-empty
 * @returns {Promise<{ context: object, formatPrefs: object }>}
 */
export async function buildBulkChargeSetContext(svc, tenantId, chargeSetIds) {
  ...
  // Load all charge-sets — mirror buildChargeSetContext's SELECT columns
  // (id, charge_set_number, total_cents, created_at, order_id) for bulk aggregation.
  // Tenant ownership is verified at query time by scoping the WHERE clause.
  const { data: chargeSets, error } = await svc
    .from('order_charge_sets')
    .select('id, charge_set_number, total_cents, created_at, order_id')
    .eq('tenant_id', tenantId)
    .in('id', chargeSetIds)
    .order('created_at', { ascending: true });
  ...
  // buildChargeSetContext signature: (svc, chargeSetId, tenantId) — chargeSetId before tenantId.
  const { context: singleCtx, formatPrefs } = await buildChargeSetContext(
    svc, chargeSetIds[0], tenantId
  );

  // Compute bulk-specific values
  const chargeSetNumbers = chargeSets
    .map((c) => c.charge_set_number)
    .filter(Boolean)
    .join(', ');
  ...
  const context = {
    ...singleCtx,
    charge_sets: chargeSets, // plural — full array for future tokens / debugging
    charge_set: {
      ...singleCtx.charge_set, // fields from first charge-set: number, total, reference_number, order
      numbers: chargeSetNumbers,    // bulk token: comma-joined charge-set numbers
 ...
 *     charge_set_total (dollars, string with 2 decimals),
 *     charge_set_number
 *   - Inherited from parent order: load_number, customer_name, driver_name
 *     (populated via minimal order lookup using cs.order_id)
 *
 * Thin standalone adapter — does NOT reuse buildChargeSetContext (which
 * is AR-invoice-oriented with invoice-specific fields irrelevant to
 * status triggers). Returns `orderId` for downstream log-keying.
 *
...
export async function buildChargeSetTriggerContext(svc, { tenantId, chargeSetId, userId }) {
  ...
  // 2. Fetch parent order (minimal joins — templates reference load_number
  //    + customer_name + driver_name most frequently)
  let orderVars = {};
  if (cs.order_id) {
    try {
      ...
      if (order) {
        const driverDisplay =
          order.driver?.name ||
          [order.driver?.first_name, order.driver?.last_name].filter(Boolean).join(' ') ||
          '';
        orderVars = {
          load_number: String(order.load_number || ''),
          customer_name: String(order.customer?.name || ''),
          driver_name: String(driverDisplay),
        };
      }
    } catch (e) {
      console.warn(`buildChargeSetTriggerContext: parent order fetch failed for charge_set ${chargeSetId}:`, e?.message || e);
      // Continue with empty parent-order vars — templates render blanks.
    }
  }
  ...
  const csVars = {
    ...
    // Dollars, 2-decimal string — matches existing format conventions
    // in other currency-touching helpers.
    charge_set_total: ((cs.total_cents || 0) / 100).toFixed(2),
    charge_set_number: String(cs.charge_set_number || ''),
  };
 ...
 *
 * Variables produced:
 *   - Move-specific: move_id, move_type, move_status, move_scheduled_date,
 *     move_started_at, move_completed_at
 *   - Inherited from parent order: load_number, customer_name, driver_name
 *     (populated via minimal order lookup using move.order_id)
 *
 * Returns `orderId` for downstream log-keying (fireTrigger preserves load_id
 * keying via parent-order resolution).
 *
 * NOTE: `order_container_moves` does NOT have from/to location columns today
 * — those live on `order_routing_events`. If admins want location-in-template
 * variables, they can extend this function to join routing events.
 *
 * @param svc service-role Supabase client
...
export async function buildMoveTriggerContext(svc, { tenantId, moveId, userId }) {
  ...
  //    Intentionally lighter than buildTriggerContext's full joins —
  //    move-status templates rarely need full container/chassis/location
  //    context. Admins who want those can switch to order-triggered variants.
  let orderVars = {};
  if (move.order_id) {
    try {
      const { data: order } = await svc

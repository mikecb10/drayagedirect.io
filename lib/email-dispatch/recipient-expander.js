 ...
 *   { type: 'contact',       value: '<customer_contact uuid>' }
 *   { type: 'contact_group', value: '<customer_contact_group uuid>' }
...
async function expandEntry(svc, tenantId, entry, context, cache) {
  ...
  switch (entry.type) {
    ...
    case 'contact_group': {
      ...
      if (cache && cache.has(cacheKey)) return cache.get(cacheKey);
      const { data } = await svc
        .from('customer_contact_group_members')
        .select('contact:customer_contacts(email)')

...
export async function dispatchEmail(svc, params) {
  ...
  // Schema columns (migration 053): tenant_id, trigger_id, template_id,
  // load_id, event_name, fire_key, outcome, outcome_detail,
  // umbrella_decisions (JSONB), messages_created, fired_at.
  // Manual sends have no umbrella routing, so we use umbrella_decisions
  // as a single-entry audit payload with type: 'manual' — the shape is
  // documented as extensible in the schema comment.

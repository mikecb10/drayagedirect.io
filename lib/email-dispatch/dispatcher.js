...
export async function dispatchEmail(svc, params) {
  ...
  // Resolve From: display-name — use helper when template/config/tenant objects
  // are provided (Task 8+ callers), otherwise fall back to caller-supplied value.
  const resolvedFromName = (templateObj || configObj || tenantObj)
    ? resolveFromDisplayName(templateObj, configObj, tenantObj)
    : fromName;

  // Resolve Reply-To — use helper when config/tenant objects are provided,
  // otherwise fall back to caller-supplied replyTo / replyToName.
  let resolvedReplyTo     = replyTo;
  let resolvedReplyToName = replyToName;
  if (configObj || tenantObj) {
    const rt = resolveReplyTo(configObj, tenantObj);
    resolvedReplyTo     = rt?.email || null;
    resolvedReplyToName = rt?.name  || null;
  }

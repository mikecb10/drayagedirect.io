import { PLATFORM_FALLBACK_FROM_NAME } from './constants.js';
 ...
 *   3. tenant.name
 *   4. PLATFORM_FALLBACK_FROM_NAME (platform floor)
 *
...
export function resolveFromDisplayName(template, config, tenant) {
  const chain = [
    ...
    tenant?.name,
    PLATFORM_FALLBACK_FROM_NAME,
  ];
  for (const candidate of chain) {
    const trimmed = (candidate || '').trim();
    if (trimmed) return trimmed;
  }
  return PLATFORM_FALLBACK_FROM_NAME;
}

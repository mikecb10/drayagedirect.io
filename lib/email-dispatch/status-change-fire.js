...
export async function fireStatusChangeTriggers(svc, params) {
  const { tenantId, entityType, entityId, oldStatus, newStatus, userId, skipHistoryWrite = false } = params;
  ...
  //
  //    Callers that already wrote a history row (the Stream B.1a
  //    transition helpers transitionChargeSetStatus / transitionMoveStatus)
  //    pass skipHistoryWrite: true to avoid the duplicate row that FU-074
  //    tracked. The fireOrderStatusChangeTriggers wrapper does NOT pass
  //    the flag — orders have no transition helper, so this remains the
  //    single history-writer for that entity type.
  if (!skipHistoryWrite) {
    try {
      const historyRow = {
        tenant_id: tenantId,
        ...
        actor_type: 'system',  // Stream B.1d — trigger-fire is automation, not human action
      };
  ...
  for (const trigger of triggers) {
    ...
    // All 3 entity types fire via generalized fireTrigger (Stream B.1c
    // closes the dispatcher/context-builder gaps FU-072/073 tracked).
    // For non-order entities, fireTrigger resolves the parent order
    // internally for log-keying — logs stay order-keyed (load_id).

 ...
 *   subject, html, text,
 *   body_format?,     // 'plain' | 'html' | null — when 'plain', the
 *                     //   message goes out text/plain only (html is
 *                     //   suppressed even if present). When 'html'
 *                     //   or null, SendGrid sends both parts if
 *                     //   both are provided (multipart/alternative).
...
export async function dispatch(svc, tenantId, message) {
  ...
  // thread_key groups messages into a conversation thread. For
  // trigger-driven sends we slug by load + template; for ad-hoc AR
  // manual sends (both load_id and template_id null) there's no
  // meaningful thread yet, so emit null rather than the degenerate
  // string "load-null-null". The column (migration 053) is nullable
  // and its index filters WHERE thread_key IS NOT NULL.
  const threadKey =
    message.load_id != null || message.template_id != null
      ? `load-${message.load_id ?? 'null'}-${message.template_id ?? 'null'}`
      : null;

  // Body-format selection:
  //   'plain' → text-only; drop html so SendGrid doesn't send a
  //             multipart/alternative that tricks clients into
  //             rendering an unintended HTML part.
  //   'html'  → html-preferred; always include html, text as fallback.
  //   null    → auto; include whichever parts the caller provided.
  const textOnly = message.body_format === 'plain';
  const includeHtml = !textOnly && message.html;
  const includeText = !!message.text;
  ...
  // Forward attachments if provided. Caller shape:
  //   [{ content: Buffer | string, filename, type, disposition? }]
  // SendGrid's API expects content as base64. Map in-place.
  if (message.attachments && Array.isArray(message.attachments) && message.attachments.length > 0) {
    sgMsg.attachments = message.attachments.map((a) => {
  ...
  const { data, error: insertErr } = await svc
    ...
    .insert({
      ...
      reply_to_name: message.reply_to_name || null,

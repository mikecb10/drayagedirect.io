...
// ──────────────────────────────────────────────────────────────
// Each entry's `value` maps to a real `orders` column. Some values are
// public-alias names that resolve to a different real column via
// COLUMN_ALIAS in lib/email-dispatch/evaluators/missing-information.js
// (e.g. reference_number → customer_reference). Values without a real
// backing column were removed in the 2026-04-24 audit.
//
// Removed values (11): erd, cutoff, freight_hold, customs_hold,
// return_date, trailer_type, trailer_size, gray_container_size,
// gray_container_type, gray_chassis_size, gray_chassis_type,
// gray_chassis_owner. Old saved triggers referencing these are handled
// by REMOVED_FIELDS in the evaluator (dropped with console.warn).
export const MISSING_INFO_APPLICABLE_FIELDS = [
  { value: 'vessel_eta', label: 'Vessel ETA' },
  { value: 'last_free_day', label: 'Last Free Day' },
  { value: 'discharged_date', label: 'Discharged Date' },         // → discharge_date
  { value: 'outgate_date', label: 'Outgate Date' },
  { value: 'empty_date', label: 'Empty Date' },
  { value: 'ingate_date', label: 'Ingate Date' },
  { value: 'cutoff', label: 'Cutoff' },                            // → cutoff_date
  ...
  { value: 'scac', label: 'SCAC' },                                // → steamship_line_scac
  ...
  { value: 'temperature', label: 'Temperature' },                  // → genset_temperature
  { value: 'master_bill_of_lading', label: 'Master Bill of Lading' }, // → bill_of_lading
  { value: 'house_bill_of_lading', label: 'House Bill of Lading' },   // → house_bol
  { value: 'seal_number', label: 'Seal #' },
  { value: 'reference_number', label: 'Reference #' },             // → customer_reference
  { value: 'vessel_name', label: 'Vessel Name' },
  { value: 'voyage', label: 'Voyage' },                            // → voyage_number
  { value: 'purchase_order_number', label: 'Purchase Order #' },   // → po_numbers (TEXT[])
  ...
  { value: 'return_location', label: 'Return Location' },          // → return_location_id
  { value: 'hook_chassis_location', label: 'Hook Chassis Location' },            // → hook_chassis_location_id
  { value: 'terminate_chassis_location', label: 'Terminate Chassis Location' },  // → terminate_chassis_location_id
  { value: 'trailer', label: 'Trailer' },                          // → trailer_id
];

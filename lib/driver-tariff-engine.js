...
/**
 * Get the current amount from a driver charge profile's active version/tier.
 */
function getDriverProfileAmount(profile) {
  ...
  for (const charge of charges) {
    ...
    if (basedOn === 'ar_invoice') {
      ...
      if (!arLineItems) {
        ...
        if (chargeSets?.length > 0) {
          ...
          const { data: items } = await svc
            ...
            .select('name, total_cents, per_unit_price_cents, unit_of_measure')
      ...
      if (chargeCode) {
        ...
        // No charge code specified — use total of all AR line items
      ...
      if (chargeCode) {
        ...
        // No code — use first non-percentage driver charge
      ...
      // Default: find first non-percentage charge
      const baseCharge = charges.find((c) =>
        c !== charge && c.unit_of_measure !== 'percentage' && c.amount_cents > 0
      );
      baseCents = baseCharge?.amount_cents || 0;

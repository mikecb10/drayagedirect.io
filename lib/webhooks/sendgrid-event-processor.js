 ...
 * when its severity strictly exceeds the current value — same-severity
 * events are no-ops (same-event idempotency is enforced upstream by
 * isDuplicateEvent in the webhook handler).
 *
 * Precedence per spec: bounced > dropped > spam_reported > delivered > deferred > null
 *
 * The keys of this map are the ONLY valid values for
 * email_messages.delivery_status. Keep in sync with any future CHECK
 * constraint in supabase/migrations/085_sendgrid_delivery_webhooks.sql.
 */
 ...
 *
 * Edge case: a leading-dot input like '.abc' returns an empty string.
 * SendGrid never emits leading-dot IDs, but the behavior is defined.
 */

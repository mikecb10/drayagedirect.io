...
function formatCurrency(value, prefs) {
  if (value == null || value === '') return prefs.empty_placeholder;
  // Accept either cents (integer) or dollars (float) — if integer >= 1000
  // and value isn't explicitly a float, we can't know. Our schema stores
  // currency as cents (integer) so callers should hand us cents.
  // To avoid ambiguity we require callers to pass dollars as Number; cents
  // values should be divided by 100 before calling.
  const n = Number(value);
  if (!Number.isFinite(n)) return prefs.empty_placeholder;

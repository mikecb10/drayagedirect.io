...
/**
 * Primary charge set = `charge_set_number` does NOT end with `_<digits>`.
 * Secondary/additional charge sets carry an `_N` suffix.
 * Used by AR filters to distinguish primary vs additional bill-to customers.
 *
 * @param {{ charge_set_number?: string|null }|null|undefined} cs
 * @returns {boolean}
 */
export function isPrimaryChargeSet(cs) {
  return Boolean(cs) && !/_\d+$/.test(cs?.charge_set_number || '');
}

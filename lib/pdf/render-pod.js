...
/**
 * Find the last `deliver` event across all moves. Walks moves in
 * move_index order (already arranged by the caller) and within each
 * move walks events in sequence order, returning the last `deliver`
 * encountered. Correct for multi-move loads (street turns, drop-and-pick)
 * where per-move `sequence` numbers are NOT globally unique across moves.
 *
...
function findLastDeliverEvent(moves) {
  ...
  for (const m of (moves || [])) {
    for (const e of (m.events || [])) {
      if (e.event_type === 'deliver') last = e;
    }
  }
  return last;
}

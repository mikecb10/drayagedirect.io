...
export async function fetchStatementData(svc, customerId, tenantId, asOfDate) {
  ...
  // Note: invoices table has no customer_reference column — that lives on
  // orders. We pull it via the invoice_charge_sets junction in step 2b.
  ...
  // 2b. Pull customer_reference from each invoice's first linked order via
  //     invoice_charge_sets → order_charge_sets → orders. Multi-load
  //     consolidated invoices may have different references per order; we
  //     surface the first one (consistent with InvoiceTemplate's first-order
  //     fallback). FU-035-H5-followup-D will improve this.
  ...
  if (invoiceIds.length > 0) {
    const { data: linkRows, error: linkErr } = await svc
      ...
      .select(`
        ...
        charge_set:order_charge_sets(
          order:orders(customer_reference)
        )
      `)
    ...
    for (const row of linkRows || []) {
      if (refsByInvoice.has(row.invoice_id)) continue;  // first wins
      const ref = row.charge_set?.order?.customer_reference;
      if (ref) refsByInvoice.set(row.invoice_id, ref);
    }
  }
  ...
  const openInvoices = (invoices || []).map((inv) => ({
    ...
    customer_reference:    refsByInvoice.get(inv.id) || null,

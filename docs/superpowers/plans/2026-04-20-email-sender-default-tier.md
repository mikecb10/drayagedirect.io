...
**Goal:** Add a platform-owned email sending subdomain (`mail.drayagedirect.com`)
...
check('allows mail.drayagedirect.com', isConsumerDomain('mail.drayagedirect.com') === false);
 ...
 *
 * We deliberately do NOT fall through to noreply@mail.drayagedirect.com,
...
export const PLATFORM_SENDER_DOMAIN =
  process.env.SENDGRID_PLATFORM_SENDER_DOMAIN || 'mail.drayagedirect.com';
...
# Set to mail.drayagedirect.com in prod. In dev, either use this value
...
2. Domain: `drayagedirect.com`. Custom subdomain: `mail`.
...
```bash
dig CNAME em####.mail.drayagedirect.com +short
# Expected: u####.wl.sendgrid.net.

dig CNAME s1._domainkey.mail.drayagedirect.com +short
dig CNAME s2._domainkey.mail.drayagedirect.com +short
...
curl -sS -X POST https://api.sendgrid.com/v3/mail/send \
  ...
  -d '{
    ...
    "from": {"email": "test@mail.drayagedirect.com", "name": "DrayageDirect Test"},
...
Must see:
- `Authentication-Results: ... dkim=pass header.d=mail.drayagedirect.com` ✓
- `Authentication-Results: ... spf=pass smtp.mailfrom=mail.drayagedirect.com` ✓
...
-- Expected: 1 row, domain = 'mail.drayagedirect.com', status = 'verified'.
```
...
Expected:
- `from_address` = `<tenant-slug>@mail.drayagedirect.com`
...
`dmarc=pass` on `mail.drayagedirect.com`.
...
- `Authentication-Results: ... spf=pass smtp.mailfrom=mail.drayagedirect.com` ✓
- `Authentication-Results: ... dmarc=pass` ✓
- `From:` displays as `"<Display Name>" <<slug>@mail.drayagedirect.com>`
